#!/usr/bin/env node
/*
 * CommonJS wrapper of optimize-assets for projects with "type": "module"
 * This file mirrors scripts/optimize-assets.js but uses require() so it can
 * be executed with `node scripts/optimize-assets.cjs` even when package.json
 * sets "type": "module`.
 */
const fs = require('fs');
...
function processVideo(file) {
  return new Promise((resolve) => {
    ...
    if (!ffmpeg) {
      console.log('FFMPEG not available, skipping video', path.relative(projectRoot, file));
      return resolve();
    }
